...
class MainFrame(wx.Frame):
    ...
    def scaleImage(self):
        ...
        self.img  = GetPILResizedWxImage(self.filepath, frmWidth, frmHeight)
    ...
    def startSlideshow(self):
        if not self.slideshowWorker:
            EVT_RESULT(self, self.onSlideshowStop)
            EVT_PROGRESS(self, self.onSlideshowTick)
            self.slideshowWorker = Slideshow(self, interval=5, direction="forward")

    def onSlideshowStop(self, event):
        self.slideshowWorker = None
    
    def onSlideshowTick(self, event):
